-- ==========================================
-- ADS POSTING SYSTEM - FIXED DATABASE SCHEMA
-- ==========================================
-- Unified pay-per-ad system for all user types
-- No subscription or user-type-specific pricing
-- ==========================================
...
-- Insert default user types (updated descriptions)
INSERT INTO user_types (name, description) VALUES
  ('independent', 'Independent Escort - Pay per ad'),
  ('agency', 'Agency - Pay per ad')
...
-- ==========================================
-- 3. AD PLANS TABLE (UNIFIED PRICING)
-- ==========================================
-- REMOVED user_type_id dependency - plans are now universal
...
-- Insert default ad plans (unified for all users)
INSERT INTO ad_plans (name, duration_days, price, features, sort_order) VALUES
  ('10 Days Plan', 10, 500.00, '{"ad_limit": 1, "video_required": true, "featured": false}', 1),
  ('20 Days Plan', 20, 900.00, '{"ad_limit": 1, "video_required": true, "featured": false}', 2),
  ('30 Days Plan', 30, 1200.00, '{"ad_limit": 1, "video_required": true, "featured": false}', 3)
...
ALTER TABLE user_ads 
  DROP CONSTRAINT IF EXISTS fk_user_ads_payment_id;
...
DROP TRIGGER IF EXISTS update_ad_plans_updated_at ON ad_plans;
...
-- 1. user_subscriptions table (deprecated - no longer used)
-- 2. user_type_id from ad_plans table (unified pricing)
-- 3. Subscription-related triggers and constraints
--
-- CHANGES FROM OLD SCHEMA:
-- 1. ad_plans.user_type_id removed - plans are now universal
-- 2. user_types descriptions updated to reflect pay-per-ad model
-- 3. Added payment_statistics view
-- 4. Simplified foreign key relationships
--
-- TO MIGRATE FROM OLD SCHEMA:
-- 1. Run: ALTER TABLE ad_plans DROP COLUMN IF EXISTS user_type_id;
-- 2. Run: DROP TABLE IF EXISTS user_subscriptions;
-- 3. Update existing plans to remove user_type_id references
-- 4. Run this file to create new structure
-- ==========================================

-- ==========================================
-- SCHEMA COMPLETE - UNIFIED PAY-PER-AD SYSTEM
-- ==========================================
