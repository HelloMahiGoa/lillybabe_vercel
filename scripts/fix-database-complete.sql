-- Complete Database Fix Script for LillyBabe Admin Panel
-- Run this in Supabase SQL Editor to fix all missing tables and columns

-- =====================================================
-- 1. DROP EXISTING TABLES (if they exist) - SAFE RESET
-- =====================================================
...
-- =====================================================
-- 2. CREATE TABLES WITH ALL REQUIRED COLUMNS
-- =====================================================
...
DROP POLICY IF EXISTS "Admins can manage all profile images" ON profile_images;
DROP POLICY IF EXISTS "Public can view profile images" ON profile_images;
DROP POLICY IF EXISTS "Admins can manage all PDF uploads" ON pdf_uploads;
...
DROP POLICY IF EXISTS "Admins can view all profile clicks" ON profile_clicks;
...
DROP INDEX IF EXISTS idx_profiles_is_featured;
DROP INDEX IF EXISTS idx_profiles_created_at;
DROP INDEX IF EXISTS idx_profile_images_profile_id;
DROP INDEX IF EXISTS idx_profile_images_is_primary;
DROP INDEX IF EXISTS idx_pdf_uploads_status;
DROP INDEX IF EXISTS idx_pdf_uploads_created_at;
DROP INDEX IF EXISTS idx_profile_views_profile_id;
DROP INDEX IF EXISTS idx_profile_views_viewed_at;
DROP INDEX IF EXISTS idx_profile_clicks_profile_id;
DROP INDEX IF EXISTS idx_profile_clicks_clicked_at;
...
DROP FUNCTION IF EXISTS increment_profile_views() CASCADE;
DROP FUNCTION IF EXISTS increment_profile_clicks() CASCADE;
...
DROP TRIGGER IF EXISTS increment_profile_views_trigger ON profile_views;
DROP TRIGGER IF EXISTS increment_profile_clicks_trigger ON profile_clicks;
...
SELECT schemaname, tablename, rowsecurity FROM pg_tables WHERE schemaname = 'public' AND tablename IN ('admin_users', 'profiles', 'profile_images', 'pdf_uploads', 'profile_views', 'profile_clicks');

-- Check policies exist
SELECT tablename, policyname FROM pg_policies WHERE schemaname = 'public' AND tablename IN ('admin_users', 'profiles', 'profile_images', 'pdf_uploads', 'profile_views', 'profile_clicks');

-- Check triggers exist
SELECT trigger_name, event_object_table FROM information_schema.triggers WHERE trigger_schema = 'public' AND event_object_table IN ('admin_users', 'profiles', 'profile_views', 'profile_clicks');

-- Check functions exist
SELECT routine_name FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name IN ('update_updated_at_column', 'increment_profile_views', 'increment_profile_clicks');
